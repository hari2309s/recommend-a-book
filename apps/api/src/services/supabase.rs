...
mod tests {
    ...
    async fn test_get_book() {
        let client = SupabaseClient::mock();
        let result = client.get_book::<TestBook>("test-id").await;
        // This will fail as we're using a mock client
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_select_by_user_id() {
        let client = SupabaseClient::mock();
        let result = client
            .select_by_user_id::<TestBook>("books", "test-user", Some(10))
            .await;
        // This will fail as we're using a mock client
        assert!(result.is_err());
    }
    ...
    async fn test_insert() {
        let client = SupabaseClient::mock();
        let test_book = TestBook {
            id: "test-id".to_string(),
            title: "Test Book".to_string(),
        };
        let result = client.insert("books", &test_book).await;
        // This will fail as we're using a mock client
        assert!(result.is_err());
    }
}
