...
mod tests {
    ...
    async fn test_parse_query_intent() {
        ...
        let intent = service.parse_query_intent("books by John Smith");
        assert!(matches!(
            intent,
            QueryIntent::Author { name, .. } if name == "John Smith"
        ));

        // Test genre patterns
        let intent = service.parse_query_intent("recommend mystery books");
        assert!(matches!(
            intent,
            QueryIntent::Genre { genre, .. } if genre == "mystery"
        ));

        // Test similar-to patterns
        let intent = service.parse_query_intent("books similar to The Hobbit");
        assert!(matches!(
            intent,
            QueryIntent::SimilarTo { reference, .. } if reference == "The Hobbit"
        ));

        // Test general query
        let intent = service.parse_query_intent("interesting books about space");
        assert!(matches!(
            intent,
            QueryIntent::General { query } if query == "interesting books about space"
        ));
    }
